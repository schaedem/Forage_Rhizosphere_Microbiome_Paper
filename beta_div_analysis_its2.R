...
burera_dist <- make_dist("burera_ITS2_dist.rds", meta)
karama_dist <- make_dist("karama_ITS2_dist.rds", meta)
rubona_dist <- make_dist("rubona_ITS2_dist.rds", meta)  
...
summary(r_beta_mod)
car::Anova(r_beta_mod) #species is not significant (p=0.9)!
...
b_means <- emmeans(b_beta_mod, list(pairwise~species_a), adjust="tukey")$`pairwise differences of species_a` %>%
  as_tibble() %>%
  filter(p.value < 0.06)
b_means #brach/des and des/maize are sig
...
karama_bray_plot <- 
  ...
  geom_text(data=k_letters, aes(label=`.group`, x=species_a), y=0.9, inherit.aes=FALSE)
...
ggsave("its2_bray_curtis_diff_intercrop.tiff", dpi=300, height = 8, width=10, units="in", bg="white")

...
meta <- read_csv("final_samps.csv") %>%
  ...
  dplyr::select(sample, location, species, timepoint, block, intercrop)

#read in otu tables
shared <- readRDS("final_shared_rare.rds")
shared <- shared %>%
  inner_join(meta, by="sample")
#separate dataframes by location
...
wlra_fun <- function(shared) {
  #Format matrix
  shared_wide <- shared %>%
    dplyr::select(-c(location, species, timepoint, block, intercrop)) %>%
...
#Burera
burera_wlra <- wlra_fun(b_shared) #axis1 =10.1 , axis2=7.26, axis3 =4.73 
bscores <- burera_wlra$row_scores %>% rownames_to_column("sample")
head(bscores)
bscores_otu <- burera_wlra$col_scores %>% rownames_to_column("name")
plot_ord(burera_wlra$wlra, axes=c(1,3))


#Karama
karama_wlra <- wlra_fun(k_shared) #axis1 =13, axis2 = 5.06, axis3 =3.92 
kscores <- karama_wlra$row_scores %>% rownames_to_column("sample")
kscores_otu <- karama_wlra$col_scores %>% rownames_to_column("name")
head(kscores)
ord_scree(karama_wlra$wlra)
plot_ord(karama_wlra$wlra ,axes=c(1,3))

#Rubona
rubona_wlra <- wlra_fun(r_shared) #axis1 = 10.3, axis2 =5.61, axis3 = 3.87
rscores <- rubona_wlra$row_scores %>% rownames_to_column("sample")
rscores_otu <- karama_wlra$col_scores %>% rownames_to_column("name")
head(rscores)
ord_scree(rubona_wlra$wlra)
plot_ord(rubona_wlra$wlra ,axes=c(1,2))

#save results
setwd("~/Library/Mobile Documents/com~apple~CloudDocs/Marie/Thesis/Rwanda/Lab work/04_Station_BNF/final_data/seq_data/16S/cla_data")
